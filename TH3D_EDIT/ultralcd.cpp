...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  //Hydrogel printing function
  void _lcd_move_he() {
	   const char* name = PSTR(MSG_MOVE_HE);
	   AxisEnum axis = HE_AXIS;
	   if (use_click()) { return lcd_goto_previous_menu_no_defer(); }
	   ENCODER_DIRECTION_NORMAL();
	   if (encoderPosition && !processing_manual_move) {

		   // Start with no limits to movement
		   float min = current_position[E_CART] - 1000,
		   ...
		   manual_move_offset += diff;
		   if ((int32_t)encoderPosition < 0)
		   NOLESS(manual_move_offset, min - current_position[E_CART]);
		   else
		   NOMORE(manual_move_offset, max - current_position[E_CART]);
		   #else
		   current_position[E_CART] += diff;
		   if ((int32_t)encoderPosition < 0)
		   NOLESS(current_position[E_CART], min);
		   else
		   NOMORE(current_position[E_CART], max);
		   #endif

		   manual_move_to_current(E_CART);
		   lcdDrawUpdate = LCDVIEW_REDRAW_NOW;
	   }
	   encoderPosition = 0;
	   if (lcdDrawUpdate) {
		   const float pos = NATIVE_TO_LOGICAL(processing_manual_move ? destination[E_CART] : current_position[E_CART]
		   #if IS_KINEMATIC
		   + manual_move_offset
		   #endif
		   , E_CART);
  ...
  void _lcd_move_distance_menu(const AxisEnum axis, const screenFunc_t func) {
    ...
    if (LCD_HEIGHT >= 4) {
      switch (axis) {
		...
		case HE_AXIS:
		  STATIC_ITEM(MSG_MOVE_HE, true, true); break;
        default:
  ...
  //Hydrogel get amount
  void lcd_move_get_he_amount()       { _lcd_move_distance_menu(HE_AXIS, lcd_move_he); }
  #if E_MANUAL > 1
  ...
  void lcd_move_menu() {
    ...
    #endif
	//Hydrogel Manual Extruder Movement
	MENU_ITEM(submenu, MSG_MOVE_HE, lcd_move_get_he_amount);
